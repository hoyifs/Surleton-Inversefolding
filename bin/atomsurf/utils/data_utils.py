...
class GraphLoader:
    ...
    def load(self, graph_name):
        ...
        try:
            ...
            feature_keys = self.config.feat_keys
            if self.use_esm:
                esm_feats_path = os.path.join(self.esm_dir, f"{graph_name}_if1.pt")###
...
def pdb_to_surf(pdb_path, radius, surface_dump, face_reduction_rate=0.1, max_vert_number=100000, use_pymesh=None,
                recompute_s=False):
    """
    ...
    try:
        if recompute_s or not os.path.exists(surface_dump):
            ...
            surface = SurfaceObject.from_pdb_path(pdb_path, radius, face_reduction_rate=face_reduction_rate,
...
class PreprocessDataset(Dataset):
    ...
    def __init__(self, data_dir, radius, recompute_s=False, recompute_g=False, do_agraph=False,
    ...
    def path_to_surf(self, pdb_path, surface_dump):
        return pdb_to_surf(pdb_path, radius = self.radius, surface_dump=surface_dump,
